...
#### Алгоритм установки:
1) wg-mgr.sh prepare -c config.conf -i wg0 --ip4 10.18.18.1/24
...
2. Готовит файл с сохраненными аргументами командной строки, имя задается жестко в скрипте **./last-args.conf**. Сохраняются некоторые параметры, чтобы при следующих запусках не вспоминать их значение каждый раз\
Например:
    ...
    ***name_service*** - имя сервиса (wg, awg) ***-s (--service)***
...
***-i, --wg-nic*** *\<IFACE\>* - имя внутреннего сетевого интерфейса, задает значение для **INST_SERVER_WG_NIC** в файле -config\
...
```
./wg-mgr.sh prepare -c c2.conf --ip4 1.1.1.1/24 --ip6 fd00::1/116 -i wg3 -e 0.0.0.0/1
```
...
***./last-args.conf*** - жестко заданное в скрипте, имя файла где хранятся аргументы для командной строки. Он создается при выполнении ***prepare*** или при выполнении ***install*** и обновляется при использовании ключа ***-u (--update-args)***
...
***-u, --update-args*** - флаг, что надо обновить файл с аргументами ***./last-args.conf*** соответственно текущим аргументам командной строки
...
***-o, --out-path** <path_out>  - путь куда будут записываться файлы клиента, если каталога нет, то он создается. По-умолчанию: --wg-path/.clients
...
***-o, --out-path** <path_out>  - путь куда записываются файлы клиента. По-умолчанию: --wg-path/.clients
...
```
добавить клиента с миенем CLN2 для iface wg3 ipv4 10.18.18.2/24 и с ipv6 fd01:dd00::2/64, настройки сервера взять из файла ~/test-wg/wg3.conf, файл параметров инсталляции ~/test-wg/par.conf и создать файлы для клиента в каталоге ~/test-wg/out
```
./wg-mgr.sh client -a new -i wg3 --ip4 10.18.18.2/24 --use-ipv6 --ip6 fd01:dd00::2/64 -w ~/test-wg -p par.conf --out-path ~/test-wg/out
```
