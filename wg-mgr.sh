...
wg_install() {
    ...
    if command -v ipcalc > /dev/null; then
        ...
        local _net="$(get_value_hand_param "WG_NET6" "")"
    fi
    {
        ...
        printf "%s\n" "  ### IP адрес шлюза провайдера VPS,"
        printf "%s\n" "  ### можно указать вручную, если он не определяется автоматически в процессе работы cкрипта\n"
        printf "%s\n" "  # PROVIDER_GW_IP=87.87.87.87"
        printf "%s\n" "$(get_value_hand_param "PROVIDER_GW_IP" "")"
        printf "%s\n" "  ### параметр для файла nft.rules, используется для указания counter в правилах nftables"
        printf "%s\n" "  # NFT_COUNTER=counter"
        printf "%s\n" "$(get_value_hand_param "NFT_COUNTER" "counter")"
        printf "%s\n" "  ### IP сети, которые будут натиться"
        ...
        printf "%s\n" "  ### Список адресов, которым разрешен доступ только к LAN'ам"
        printf "%s\n" "  # NFT_LIST_VPN_ONLY=\"192.168.15.12, 192.168.15.13, 192.168.16.0/24\""
        printf "%s\n" "$(get_value_hand_param "NFT_LIST_VPN_ONLY" "")"
        printf "%s\n" "  ### Список адресов, которым запрещен доступ к INET'у"
        printf "%s\n" "  # NFT_LIST_INET_DROP=\"192.168.15.14, 192.168.15.15, 192.168.17.0/24\""
        printf "%s\n" "$(get_value_hand_param "NFT_LIST_INET_DROP" "")"
        printf "%s\n" "  ### DNS сервер локальной сети"
        printf "%s\n" "  # NFT_DNS_LOCALNET=192.168.18.33"
        printf "%s\n" "$(get_value_hand_param "NFT_DNS_LOCALNET" "")"
  ...
  #"
        ...
        printf "%s\n" "  ### Список разрешенных сервисов на сервере на внешнем интерфейсе"
        printf "%s\n" "  # NFT_ALLOWED_SERVICES='tcp . 22, tcp . 3306, udp . 53'"
        printf "%s\n" "$(get_value_hand_param "NFT_ALLOWED_SERVICES" "")"
        printf "%s\n" "  ### Список разрешенных сервисов на сервере на внутреннем интерфейсе"
        printf "%s\n" "  # NFT_ALLOWED_SERVICES_VPN='tcp . 22, tcp . 3306, udp . 53'"
        printf "%s\n" "$(get_value_hand_param "NFT_ALLOWED_SERVICES_VPN" "")"
